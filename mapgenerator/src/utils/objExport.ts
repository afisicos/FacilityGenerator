...
// Helper function to generate safe filename from polygon name
function getSafeFilename(polygon: WallPolygon, prefix: string): string {
  const baseName = polygon.name || polygon.id;
  // Remove invalid filename characters and limit length
  const safeName = baseName
    .replace(/[<>:"/\\|?*]/g, '_')
    .replace(/\s+/g, '_')
    .substring(0, 50);
  return `${prefix}_${safeName}.obj`;
}
...
export function exportToOBJ(
  ...
  if (!exportTogether) {
    ...
    exportWallsSeparately(polygons, wallHeight, wallThickness);
    return;
  }
  
  // Original code: export together
...
function exportWallsSeparately(
  polygons: WallPolygon[],
  wallHeight: number,
  wallThickness: number
): void {
  ...
  // Export each polygon separately using the same logic as exportToOBJ
  polygons.forEach((polygon) => {
    const thickness = wallThickness * GRID_SIZE;
    const height = wallHeight;

    // Calculate geometric center of this polygon
    let minX = Infinity, maxX = -Infinity;
    let minY = Infinity, maxY = -Infinity;
    ...
    if (outerPoints.length > 0 && innerPoints.length > 0) {
      ...
      combinedPolygon.push([outerPoints[0][0], outerPoints[0][1]] as [number, number]);
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = getSafeFilename(polygon, 'Wall');
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
...
function exportFloorsTogether(polygons: WallPolygon[], withVolume: boolean = false): void {
  ...
  // Calculate geometric center of all polygons
  let minX = Infinity, maxX = -Infinity;
  let minY = Infinity, maxY = -Infinity;
  ...
  // Convert each polygon to a closed polygon
  ...
  // Process each polygon from union result
  unionResult.forEach(polygon => {
    const exteriorRing = polygon[0];
    const holes = polygon.slice(1);
